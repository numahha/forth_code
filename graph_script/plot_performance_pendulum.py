...
def total_cost_meta(dirname='./', trial=1, c_zerodot=0):
    ret_val=0.
    for i in range(1,loop_num):
        data = np.loadtxt(dirname+'c'+str(c_zerodot)+'_meta_trial'+str(trial)+'/result_apply/cost_'+str(i)+'.csv',delimiter=',')
        ret_val += data.sum()
    return ret_val
...
def total_cost_pro(dirname='./', trial=1, c_zerodot=0):
    ret_val=0.
    for i in range(1,loop_num):
        data = np.loadtxt(dirname+'c'+str(c_zerodot)+'_proposed_trial'+str(trial)+'/result_apply/cost_'+str(i)+'.csv',delimiter=',')
        ret_val += data.sum()
    return ret_val
...
plt.errorbar(c_list,meta,yerr=meta_std,capsize=5,color="C0",label='Plain')
plt.errorbar(c_list,pro,yerr=pro_std,capsize=5,color="C3",label='Proposed')

plt.xlabel('Friction coefficient (modeling error of PM)', fontsize=18)
plt.ylabel('Episode cost', fontsize=18)
plt.legend(fontsize=12)
plt.savefig('performance_pendulum.pdf')
plt.savefig('performance_pendulum.svg')
plt.show()
